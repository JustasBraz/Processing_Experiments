...
void draw(){
 ...
 for(int i=0; i+1<vectors.size(); i++){
   ...
   if((i>((float)vectors.size()/4))&&(i<(vectors.size()/2))){
                stroke(200,0,50);
                }
                   
   if((i>((float)vectors.size()/2))&&(i<(vectors.size()*(.75)))){
                  stroke(0,255,0);               
                }

   if((i>((float)vectors.size()*(.75)))&&(i<(float)vectors.size())){
                  stroke(0,0,255); 
                  }
...
void data(IntList linez){
 ...
 void remake(IntList linez){
     ...
     float z=0;
      ...
      for(int n=0; n<linez.size();n++){
       
         if(n<vectors.size()/4){
                 z+=0.1;
               }
                if(n==(int)vectors.size()/4){z=0;}  
                
   if((n>((float)vectors.size()/4))&&(n<(vectors.size()/2))){
               z+=0.1;
                }
                   if(n==(int)vectors.size()/2){z=0;} 
   if((n>((float)vectors.size()/2))&&(n<(vectors.size()*(.75)))){
                 z+=0.1;              
                }
      if(n==(int)vectors.size()*(0.75)){z=0;} 
   if((n>((float)vectors.size()*(.75)))&&(n<(float)vectors.size())){
                 z+=0.1;
                  }
